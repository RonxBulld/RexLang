...
namespace opene {
    ...
    bool TypeAssert::IsAssignableBetweenType(const TypeDecl * lhs_type, const TypeDecl * rhs_type) {
        ...
        assert(rhs_type);
        {
            const ArrayDecl *lhs_array = lhs_type->as<ArrayDecl>();
            const ArrayDecl *rhs_array = rhs_type->as<ArrayDecl>();
            // 1. 至少有一个是数组
            if (lhs_array || rhs_array) {
                if (lhs_array && rhs_array) {
                    // 1.1. 两者都是数组则判定基础类型
                    return TypeAssert::IsAssignableBetweenType(lhs_array->base_type_, rhs_array->base_type_);
                } else {
                    // 1.2. 其中一个是数组则不可赋值
                    assert(false);
                    return false;
